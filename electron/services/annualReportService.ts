...
class AnnualReportService {
  ...
  private async computeLongestStreak(
    ...
    for (let i = 0; i < sessionIds.length; i++) {
      ...
      const cursor = await wcdbService.openMessageCursor(sessionId, 2000, true, beginTimestamp, endTimestamp)
  ...
  async generateReportWithConfig(params: {
    ...
    try {
      ...
      if (!useSqlExtras) {
        ...
        for (let i = 0; i < sessionIds.length; i++) {
          const sessionId = sessionIds[i]
          const cursor = await wcdbService.openMessageCursor(sessionId, 1000, true, actualStartTime, actualEndTime)
          if (!cursor.success || !cursor.cursor) continue
